 ...
 */
function formatCSSVariableName(collectionName, variableName) {
...
class ComponentCollector {
     ...
     */
    static async collectNodes(node, componentsData, componentSets) {
        if ("type" in node) {
...
ComponentCollector.componentMap = new Map();
// ================== CSS GENERATOR ==================
/**
...
class CSSGenerator {
     ...
     */
    static async generate(collections) {
        ...
        for (const collection of collections) {
            ...
            for (const variable of collection.variables) {
                ...
                if (value &&
                    ...
                    const semanticVar = await this.handleVariableAlias(variableFromFigma, collection.name, value.id);
                    if (semanticVar) {
                ...
                const cssVar = this.formatVariable(collection.name, variableFromFigma.name, value, variableFromFigma.resolvedType);
                if (cssVar) {
     ...
     */
    static async handleVariableAlias(variable, collectionName, referencedVariableId) {
        ...
        const referencedVarName = formatCSSVariableName(referencedVarCollection.name, referencedVariable.name);
     ...
     */
    static formatVariable(collectionName, variableName, value, variableType) {
     ...
     */
    static isSizeValue(variableName) {
...
class TestGenerator {
     ...
     */
    static generate(component, generateAllVariants = false) {
        ...
        if (isComponentSet) {
            ...
            if (defaultVariant) {
                ...
                testContent = this.createTestWithStyleChecks(componentName, kebabName, variantStyleChecks);
            }
            else {
                testContent = this.createTestWithStyleChecks(componentName, kebabName, styleChecks);
            }
        }
        else {
...
`;
     ...
     */
    static extractStyleChecks(styles) {
        const styleChecks = [];
     ...
     */
    static generateStyleChecks(styleChecks) {
     ...
     */
    static createTestWithStyleChecks(componentName, kebabName, styleChecks) {
        const pascalName = toPascalCase(componentName);
...
class GitLabAPI {
     ...
     */
    static async commitFile(options) {
        const { projectId, gitlabToken, commitMessage, filePath, branchName = "main", content } = options;
     ...
     */
    static async commitTestFile(options, progressCallback) {
        try {
     ...
     */
    static async checkFileExists(projectId, gitlabToken, filePath, branchName) {
     ...
     */
    static async getFileInfo(projectId, gitlabToken, filePath, branchName) {
        const checkFileUrl = `${GITLAB_API_BASE}/projects/${projectId}/repository/files/${encodeURIComponent(filePath)}?ref=${branchName}`;
...
class SettingsManager {
     ...
     */
    static async saveGitLabSettings(settings, shareWithTeam = false) {
        ...
        if (shareWithTeam) {
            ...
            figma.root.setSharedPluginData(PLUGIN_NAME, "gitlab-settings", JSON.stringify(settingsWithMetadata));
     ...
     */
    static async loadGitLabSettings() {
        try {
            // Try to load settings from the document (available to all team members)
            const documentSettings = await figma.root.getSharedPluginData(PLUGIN_NAME, "gitlab-settings");
            if (documentSettings) {
                try {
     ...
     */
    static async clearGitLabSettings() {
        try {
 ...
 */
async function handleCSSExport(msg) {
    try {
 ...
 */
async function handleTestGeneration(msg) {
    try {
 ...
 */
async function handleSaveGitLabSettings(msg) {
    try {
        const settings = {
 ...
 */
async function handleGitLabCommit(msg) {
    try {
 ...
 */
async function handleGitLabTestCommit(msg) {
    try {
