...
async function buildUI() {
  try {
    ...
    if (!isDev) {
      ...
      console.log(
        `   CSS: ${(minifiedCSS.length / 1024).toFixed(2)} KB (${((1 - minifiedCSS.length / css.length) * 100).toFixed(1)}% reduction)`,
      );
      console.log(
        `   JS: ${(minifiedJS.length / 1024).toFixed(2)} KB (${((1 - minifiedJS.length / js.length) * 100).toFixed(1)}% reduction)`,
      );
    }
    ...
    if (!isDev) {
      ...
      const savings = (
        (1 - output.length / (css.length + js.length + bodyHtml.length)) *
        100
      ).toFixed(1);
