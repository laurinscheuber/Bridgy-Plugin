...
window.onmessage = (event) => {
  try {
    ...
    if (message.type === 'gitlab-settings-loaded') {
      ...
      // Re-analyze component hygiene to update the list
      analyzeComponentHygiene();
...
function displayComponentHygieneSection(result) {
  ...
  // Render unused components collapsible
  const hygieneGroupId = 'component-hygiene-group';
  const isExpanded = true; // Auto-expand by default

  html += `
    <div id="${hygieneGroupId}" class="variable-collection quality-collection" style="margin-bottom: 12px;">
      <div class="collection-header header ${isExpanded ? '' : 'collapsed'}" onclick="toggleCollection('${hygieneGroupId}')">
        <div class="collection-info">
          <div class="collection-name-title" style="display: flex; align-items: center; gap: 8px; font-size: 13px;">
            <span class="material-symbols-outlined" style="font-size: 18px; margin-right: 6px; color: var(--purple-light);">warning</span>
            Unused Components
            <span class="subgroup-stats">${result.unusedCount}</span>
          </div>
        </div>
        <span class="material-symbols-outlined collection-toggle-icon">expand_more</span>
      </div>
      <div id="${hygieneGroupId}-content" class="collection-content ${isExpanded ? '' : 'collapsed'}">
        <div style="padding: 12px;">
  `;
  ...
  result.unusedComponents.forEach((component, idx) => {
    ...
    if (hasVariants && !isFullyUnused) {
      ...
      componentHtml += `
        <div class="node-header" onclick="toggleComponentHygieneGroup('${groupId}')" style="display: flex; align-items: center; cursor: pointer;">
