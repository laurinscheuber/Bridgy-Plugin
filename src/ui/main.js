        ...
        if (itemCard) {
          ...
          setTimeout(() => {
            ...
            if (window.tailwindActiveCollectionId) {
              ...
              if (collection) {
                // Count visible (non-hidden) issue cards in this collection
                const allIssueCards = collection.querySelectorAll('.quality-issue-card');
                const visibleIssueCards = Array.from(allIssueCards).filter(card => {
                  return card.style.display !== 'none';
                });
                const visibleCount = visibleIssueCards.length;
                ...
                // If no visible issues remain, show a success message
                if (visibleCount === 0) {
                  const contentDiv = collection.querySelector('.collection-content > div');
                  if (contentDiv) {
                    // Hide all the hidden cards completely
