  ...
  try {
    ...
    if (message.type === 'gitlab-settings-loaded') {
      ...
      showNotification(
        'success',
        'Component Deleted',
        `Component "${message.componentName}" deleted successfully`,
      );
      ...
      showNotification('error', 'Delete Failed', `Failed to delete component: ${message.error}`);
...
function displayComponentHygieneSection(result) {
  ...
  result.unusedComponents.forEach((component, idx) => {
    ...
    const componentType = component.type === 'COMPONENT_SET' ? 'Component Set' : 'Component';
    const variantInfo = component.type === 'COMPONENT_SET' ? ` (${component.variantCount} variant${component.variantCount !== 1 ? 's' : ''})` : '';
