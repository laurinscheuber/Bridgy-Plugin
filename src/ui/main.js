...
window.onmessage = (event) => {
  try {
    ...
    if (message.type === 'gitlab-settings-loaded') {
      ...
      // Remove the fixed issue from the DOM
      if (window.tailwindProcessingItemId) {
        ...
        if (itemCard) {
          ...
          setTimeout(() => {
            ...
            // After removal, check if the collection is empty
            if (window.tailwindActiveCollectionId) {
              const collection = document.getElementById(window.tailwindActiveCollectionId);
              if (collection) {
                const issuesContainer = collection.querySelector('.collection-content > div');
                const remainingIssues = issuesContainer ? issuesContainer.querySelectorAll('.quality-issue-card').length : 0;

                // If no issues remain in this collection, we might need to re-render
                // But for now, just keep the collection open and empty
                if (remainingIssues === 0) {
                  // Optionally add an empty state message
                  if (issuesContainer) {
                    issuesContainer.innerHTML = `
                      <div style="text-align: center; padding: 20px; color: rgba(255, 255, 255, 0.5); font-size: 13px;">
                        <span class="material-symbols-outlined" style="font-size: 24px; display: block; margin-bottom: 8px; opacity: 0.5;">check_circle</span>
