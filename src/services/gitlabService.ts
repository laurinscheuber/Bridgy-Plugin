...
import {
  ...
  ERROR_MESSAGES,
  LoggingService,
  buildGitLabApiUrlSync,
  buildGitLabWebUrlSync,
} from '../config';
...
export class GitLabService {
  ...
  static async saveSettings(settings: GitLabSettings, shareWithTeam: boolean, shareTokenWithTeam: boolean = false): Promise<void> {
    ...
    try {
      ...
      if (!shareWithTeam) {
        // Save to personal storage only (strip token – saved separately encrypted)
        ...
        try {
          if (CryptoService.isAvailable()) {
            ...
            if ((settings as any)._needsCryptoMigration) {
              await figma.clientStorage.deleteAsync(`${clientKey}-key`);
  ...
  static async loadSettings(): Promise<GitLabSettings | null> {
    try {
      ...
      // Helper: try loading encrypted token from clientStorage (new key, then legacy)
      const loadPersonalToken = async (settings: GitLabSettings): Promise<void> => {
        for (const key of [clientKey, legacyClientKey]) {
          ...
          if (settings.gitlabToken) return; // found token, stop
        }
      };
      ...
      if (documentSettings) {
        try {
          ...
          if ((settings as any).shareTokenWithTeam && tokenShared === 'true') {
      ...
      for (const key of [clientKey, legacyClientKey]) {
        ...
        if (personalSettings) {
          const settings = Object.assign({}, personalSettings, { isPersonal: true }) as GitLabSettings;
  ...
  static async resetSettings(): Promise<void> {
    try {
      ...
      // Remove personal client storage (new + legacy)
      for (const key of [clientKey, legacyClientKey]) {
  ...
  private static async makeAPIRequest(url: string, options: RequestInit): Promise<Response> {
    ...
    const timeoutPromise = new Promise<never>((_, reject) => {
      setTimeout(() => {
        reject(
          new GitLabNetworkError('Request timed out - please check your connection to GitLab'),
        );
      }, timeoutMs);
    });
    ...
    try {
      ...
      const response = await Promise.race([
        useProxy ? ProxyService.proxyGitLabRequest(url, options) : fetch(url, options),
        timeoutPromise,
      ]);
  ...
  private static handleGitLabError(error: any, operation: string): Error {
    if (
      ...
      error instanceof GitLabNetworkError
    ) {
      return error;
    }
      ...
      switch (error.statusCode) {
        ...
        case 403:
          return new GitLabAuthError(
            `Authentication failed while trying to ${operation}. Please check your GitLab token.`,
          );
        case 404:
          return new GitLabAPIError(
            `Resource not found while trying to ${operation}. Please check your project ID.`,
            404,
          );
        case 422:
          return new GitLabAPIError(
        ...
        case 504:
          return new GitLabAPIError(
            `GitLab server error while trying to ${operation}. Please try again later.`,
            error.statusCode,
          );
        default:
          return new GitLabAPIError(
            `GitLab API error while trying to ${operation}: ${error.message || 'Unknown error'}`,
            error.statusCode,
          );
      }
    }
