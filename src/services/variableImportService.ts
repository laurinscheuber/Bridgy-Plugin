...
export class VariableImportService {
   ...
   */
  private static createGradientStyle(token: ImportToken, stylesMap: Map<string, PaintStyle>, overwrite: boolean) {
   ...
   * Now synchronous using pre-fetched map
   */
  private static createShadowStyle(token: ImportToken, stylesMap: Map<string, EffectStyle>, overwrite: boolean) {
      const styleName = token.name.replace(/^--|\$/, '');
      let style = stylesMap.get(styleName);
  ...
  private static mapTokenTypeToFigmaType(type: ImportToken['type']): VariableResolvedDataType | null {
    switch (type) {
  ...
  private static parseRadialGradient(value: string): GradientPaint {
      ...
      return {
          ...
          gradientTransform: [[1, 0, 0.5], [0, 1, 0.5]], // Centered radial
          gradientStops: stops
      };
  }
  ...
  private static parseConicGradient(value: string): GradientPaint {
      ...
      return {
          ...
          gradientTransform: [[1, 0, 0.5], [0, 1, 0.5]], // Centered angular
          gradientStops: stops
      };
  }
  ...
  private static isColorStop(part: string): boolean {
      ...
      return trimmed.includes('#') || 
             ...
             /^[a-zA-Z]+(\s+\d+%?)?$/.test(trimmed); // Named colors with optional position
  }
