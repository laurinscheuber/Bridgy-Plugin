...
describe('GitLab Commit E2E', () => {
  ...
  describe('Settings Integration', () => {
    it('should save and load GitLab settings correctly', async () => {
      ...
      // Settings are now stored in pluginData (not sharedPluginData)
      ...
      // Mock stored settings via pluginData
      mockEnvironment.figma.root.getPluginData = vi.fn((key: string) => {
        if (key.includes('gitlab-settings') && !key.includes('-shared-token') && !key.includes('-token-shared')) {
    ...
    it('should reset settings completely', async () => {
      ...
      // Settings are now cleared via pluginData (not sharedPluginData)
      expect(mockEnvironment.figma.root.setPluginData).toHaveBeenCalledWith(
        expect.stringContaining('gitlab-settings'),
        '',
      );
