...
vi.mock('../../src/services/componentService', () => ({
  ComponentService: {
    ...
    async generateComponentTest(component: any, framework = 'react') {
      ...
      const variant = component.variants.find((v: any) => v.property === 'variant')?.value || 'Default';
...
describe('Component Reading E2E', () => {
  ...
  describe('Error Handling', () => {
    it('should handle missing components gracefully', async () => {
      const nonExistentComponent = await ComponentService.getComponentByName('NonExistent/Component');
